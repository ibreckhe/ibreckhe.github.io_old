...
setwd("~/Dropbox/Research/MORA_community_turnover/")

##Loads and formats data.
photos <- read.csv("./data/MORA_grid_500m_flickr.csv")
...
parkbound <- readShapePoly("./data/MORA_boundary_UTM10.shp")
...
m <- leaflet() %>% addTiles(MBTemplate) %>% setView(lat=46.854039, lng=-121.660366, zoom = 10)

##Adds park boundary data.
m <- m %>% addPolygons(data=parkbound_latlng,
                       color="white",fillColor="white",
                       opacity=0.7,fillOpacity=0.1)

##Adds Flickr Photo data with color scale.

##Transforms data and makes a color ramp.
...
m  <- m %>% addCircleMarkers(lng=coordinates(photos_latlng)[,1],
                 ...
                 radius=log(photos_latlng@data$PNTCNT+1)*2,
                 fillColor=cols,fillOpacity=0.9,
                 weight=0.5)

##Creates a legend image and exports to JPEG.
leg_breaks <- c(1,10,100)
leg_size <- log(leg_breaks+1)
leg_col <- rgb(ramp(trans_fun(leg_breaks)),maxColorValue=256)

png(filename="./maps/web_map_legend.png",width=50,height=70,unit="px",bg=NULL)
par(mar=c(1,1,0,0))
...
##Adds points of interest as popups markers.
...
The map shows the number of accurately geo-tagged photos uploaded to the Flickr and iNaturalist websites from 2009 to 2013. The size and color of the dots is proportional to the number of photos appearing in 500m square bins overlayed on the park, and the park boundary is shown in white.

**Installing the leaflet R package**

We will build this map up layer by layer, using data that is available <here>, but first we need to set up our R workspace:
...
```
Next we need to install the leaflet R package. It's not up on CRAN yet, but you can install it from GitHub:
```{r eval=FALSE}
require(devtools)
...
Leaflet overlays R data onto tiled web maps of various sources. To use the default tile set (OpenStreetMap), all you need to do is call the `leaflet()` function and add tiles to it. If you are using the newest version of Rstudio, then this map appears alongside your code in the viewer pane.

```{r}
leaflet() %>% addTiles()
```

What the line above does is use the `leaflet()` function to create an HTML widget object, and then uses the `addTiles` function to populate that widget with tiles from the web. 

The syntax might be a bit unfamiliar because of the pipe (`%>%`) operator. What this does is input the results of the left-hand expression as the first argument of the right-hand expression. The main advantage of using the pipe is that it allows related operations to be chained together without nesting functions using the confusing 'f(g(x))' syntax. 

**Adding custom tiles**

If your primary goal is just to explore geographic data, then the default tile set is just fine. If you want to share your map with others, however, it might be nice to have some creative control over how the tiles look. Luckily, it's pretty straight-forward to change the background map. Many web mapping services use a standard URL scheme to organize access to the individual tile images, so we can change tile sets by specifying a URL template. 

This template has a fixed portion, and then a few regions that change based on the geographic coordinates and the zoom level. For our map, we are going to use some custom tiles I designed with MapBox. To access the tiles, we need to build the URL template that includes the path to the tiles and the access token:
...
To do this, we are going to scale the size and color of the points according to the number of photos at that location. The first part of the code rescales the variable PNTCNT to a log scale that lies in the interval 0-1.
...
The vector `cols` now contains hex color values that map to the rescaled values of PNTCNT. We don't need to be quite as elaborate with our calculations of size:
```{r}
